...
int main(void) {
	...
	Why to have length of n - k + 1? Starting at n - k + 1 is crucial because it represents the beginning of the last possible segment of length K in 
 	the permutation P. */
	...
	// Finding the optimal position to sort
	for (int i = 1; i <= n - k; i++) {
		...
		if ((i <= n - 2 * k + 1) && (next_greater[i] >= i + k - 1)) { idx = min(i, idx); }
		...
		// If both conditions are met, idx is updated to the minimum of i and the current value of idx.
		else if ((next_greater[i] >= n - k + 1) && (P[n - k] < min_suffix[i + k - 1])) { idx = min(i, idx); }
	}
